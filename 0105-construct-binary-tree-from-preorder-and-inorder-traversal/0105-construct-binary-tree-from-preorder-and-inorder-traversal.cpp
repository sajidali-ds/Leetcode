...
public:
   TreeNode* builtTree(vector<int>& preorder,int preStart,int preEnd,vector<int>& inorder,int inStart,int inEnd,map<int,int>& inMap){
    ...
    root->left=builtTree(preorder,preStart+1,preStart+NumsLeft,inorder,inStart,inRoot-1,inMap);
    root->right=builtTree(preorder,preStart+NumsLeft+1,preEnd,inorder,inRoot+1,inEnd,inMap);
    return root;
